...
trixi_include(@__MODULE__, joinpath(@__DIR__, "setup_elastic.jl");
...
restart_file_structure = joinpath(output_directory, prefix * "structure_1_current.vtu")

ode_restart = semidiscretize(semi, tspan;
                             restart_with=(restart_file_fluid,
                                           restart_file_open_boundary,
                                           restart_file_structure))
