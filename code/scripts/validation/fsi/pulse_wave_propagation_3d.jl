...
young_modulus = 1e6 # corresponds to 10^7 dyn/cm^2
...
vessel = union(vessel_clamped_in, vessel_clamped_out, vessel_flexible)
...
background_pressure = 10 * sound_speed_factor / 10 * fluid_density * wave_speed^2
shifting_technique = TransportVelocityAdami(; background_pressure)

state_equation = StateEquationCole(; sound_speed, reference_density=fluid_density,
                                   exponent=1)
fluid_system = WeaklyCompressibleSPHSystem(fluid; state_equation, smoothing_kernel,
...
inflow = BoundaryZone(; boundary_face=face_in, face_normal=flow_direction,
                      ...
                      reference_pressure=5000.0, initial_condition=inlet)
...
vessel_system = TotalLagrangianSPHSystem(vessel; smoothing_kernel, smoothing_length,
                                         young_modulus, poisson_ratio=vessel_nu,
                                         boundary_model=boundary_model,
                                         penalty_force=penalty_force,
                                         viscosity=ArtificialViscosityMonaghan(alpha=0.01),
                                         clamped_particles=1:(nparticles(vessel_clamped_in) + nparticles(vessel_clamped_out)))
