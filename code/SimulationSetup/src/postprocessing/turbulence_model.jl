...
Turbulence model implementation for SPH simulations using the Smagorinsky eddy viscosity model.

This module calculates turbulent stresses, wall shear stresses, and surface normals for
fluid dynamics simulations. It computes:
- Velocity gradients from particle interactions
- Strain rate tensors
- Turbulent and laminar stress components
- Wall shear stresses and traction vectors

The model combines:
- Laminar viscous stress: τ_lam = 2μS
- Smagorinsky eddy viscosity: μ_T = ρ(C_s Δ)² |S|
- Isotropic turbulent part: -(2/3)ρC_I Δ² |S|²

Originally adapted from TrixiParticles.jl. This code should potentially be contributed back
to TrixiParticles.jl as a standard turbulence model module for SPH simulations.
"""
...
function SPSTurbulenceModelDalrymple(; smallest_length_scale, smagorinsky_constant=0.12,
                                     isotropic_constant=6.6e-3, dynamic_viscosity,
...
function calculate_fluid_stress_tensor!(system::AbstractFluidSystem,
                                        turbulence_model, v_ode, u_ode, semi)
...
function calculate_velocity_gradients!(system, velocity_gradient_tensor,
    ...
    foreach_system(semi) do neighbor_system
        ...
        foreach_point_neighbor(system, neighbor_system,
            ...
            v_a_ = current_velocity(v, system, particle)
            v_b_ = current_velocity(v_neighbor, neighbor_system, neighbor)
            ...
            v_a = viscous_velocity(v, system, particle, v_a_)
            v_b = viscous_velocity(v_neighbor, neighbor_system, neighbor, v_b_)
...
function calculate_stress_tensor!(system, turbulence_model, v, semi)
    ...
    @threaded semi for particle in each_integrated_particle(system)
        ...
        # The following three terms can be disabled by setting
        # `smagorinsky_constant`, `smallest_length_scale` or `mu` to zero
